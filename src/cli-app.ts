...
import { advancedUsage, faultLineVersion, printQuickstart, usage } from "./cli-help.js";
import {
...
async function judgeDemo(args: string[]): Promise<void> {
  ...
  const { startFaultLineServer } = await import("./server.js");
...
async function witnessProposal(args: string[]): Promise<void> {
  ...
  if (!hasFlag(args, "--live")) {
    const { defaultWitnessProposal } = await import("./ai.js");
  ...
  const { proposeWitnessWithGpt } = await import("./ai.js");
...
async function judgeProofCommand(args: string[]): Promise<void> {
  ...
  const { startGitProofServer } = await import("./server.js");
...
async function initCommand(args: string[]): Promise<void> {
  ...
  let snapshotPrewarm:
    | { status: "PREWARMED"; cachePath: string; treeDigest: string; note: string }
    | { status: "SKIPPED"; reason: string }
    | undefined;
  if (yes) {
    try {
      mkdirSync(join(repository, ".faultline"), { recursive: true });
      const cachePath = turnSnapshotRepoCachePath(repository);
      const capture = captureTurnTreeSnapshot(repository, {
      ...
      snapshotPrewarm = {
        status: "PREWARMED",
        cachePath,
        treeDigest: capture.snapshot.treeDigest,
        note: "first-turn snapshot pre-warmed"
      };
      process.stderr.write("first-turn snapshot pre-warmed\n");
    } catch (error) {
      snapshotPrewarm = {
        status: "SKIPPED",
...
export async function runCli(argv: readonly string[] = process.argv.slice(2)): Promise<void> {
  ...
  switch (command) {
    ...
    case "ui": {
      const repository = resolve(option(args, "--repo") ?? process.cwd());
      const { DEFAULT_UI_HUB_PORT, startFaultLineUiHub } = await import("./ui-hub.js");
