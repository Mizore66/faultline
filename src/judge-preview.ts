...
${renderIncidentPageProductChrome({
  ...
  ideaBeat: "This page is a fixture sandbox for the evidence model — not COMMIT_PROOF. For the product Idea, use fl judge-proof or docs/self-incident-proof-preview.html after the sample bundle is installed.",
...
<section class="section" id="prevent"><div class="section-head"><span class="number">04</span><h2>NEXT</h2></div><article class="card panel"><h3>Open the product Idea path</h3><p>Install the recorded self-incident sample under <code>docs/samples/self-incident-commit-proof/</code>, then run <code>pnpm fl judge-proof</code> or open the static COMMIT_PROOF snapshot after <code>pnpm fl commit-proof-preview</code>.</p></article></section>
${renderIncidentPageDocumentEnd({
