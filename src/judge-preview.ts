...
<main class="shell">
  ...
  <section class="hero"><div><div class="eyebrow">EXECUTABLE-EVIDENCE PRODUCT WALKTHROUGH</div><h1>Freeze the question.<br><em>Inspect the boundary.</em></h1><p class="lede">FaultLine turns one reviewed behavioral witness into a bounded investigation. This preview shows the product's decision model and its honesty boundary without presenting sample data as production evidence.</p></div><aside class="card metrics" aria-label="Sample fixture summary"><div class="metric"><b>${analysis.metrics.sessions}</b><small>sample sessions</small></div><div class="metric"><b>${analysis.metrics.turns}</b><small>sample turns</small></div><div class="metric"><b>${analysis.metrics.files}</b><small>changed files</small></div><div class="metric"><b>${analysis.metrics.implicatedHunks}</b><small>implicated hunks</small></div></aside></section>
  <section class="grid">
    ...
    <article class="card"><div class="eyebrow">05 / PROOF THRESHOLD</div><h2>Why this stays a preview</h2><p class="muted">A real FaultLine proof requires an approved frozen witness, isolated recorded executions across immutable Git states, and an independently verifiable package. This page contains none of those live artifacts.</p><ul>${proofThresholds}</ul></article>
  </section>
